document.addEventListener('DOMContentLoaded', () => {
			...
			window.renderReplies = function (replies, container) {
			    ...
			    replies.forEach(reply => {
					...
					console.log(reply.isSecret, reply.isAccessible, reply.rpContent);
			                ...
			                ` : ''}
						              ...
						              <div class="actions">
						                  ...
						                  <button onclick="toggleLike(${reply.replyId})">
						                      <span id="like-icon-${reply.replyId}" class="${reply.isLiked ? 'liked' : ''}">❤️</span>
			...
			window.toggleLike = function(replyId) {
			    const likeIcon = $(`#like-icon-${replyId}`);
			    const likeCountElement = $(`#like-count-${replyId}`);
			    ...
			    $.ajax({
			        ...
			        success: function(response) {
			            ...
			            // 좋아요 개수 갱신
			            $.get(`/reply/${replyId}/like-count`, function(likeCount) {
